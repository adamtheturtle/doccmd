...
- Use click-compose 2026.09.16.1 and its ``multi_callback`` API for option validation.
...
- Add ``--respect-gitignore`` / ``--no-respect-gitignore`` option to respect ``.gitignore`` files when discovering files in directories.
  ...
  This uses ``dulwich`` and respects ``.gitignore``, ``.git/info/exclude``, and global gitignore files.
